BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["BOM_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["BOM_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["BOM_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["BOM_NOLINEWRAP"] != "" && ENVIRON["BOM_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
